	...
	return class extends Plugin {
		...
		initialize() {
			if (typeof BDfunctionsDevilBro === "object") {
				...
				this.css = `
					.${this.id}-modal .titles {
						height: 20px;
					}
					
					.${this.id}-modal .emojiserver-entry {
						height: 50px;
						padding-top: 5px;
						padding-bottom: 5px;
					}
					
					.${this.id}-modal .emojiserver-entry .modal-emojiserver-icon {
					...
					.${this.id}-modal .titles-entry label,
					.${this.id}-modal .emojiserver-entry label {
						color: #b9bbbe;
						display: inline-block;
						flex: 1;
						font-size: 12px;
					...
					.${this.id}-modal .titles-entry .modal-titlesicon-label {
						margin-left: 0px;
						text-align: center;
						width: 50px;
					}
					
					.${this.id}-modal .titles-entry .modal-titlesservername-label,
					.${this.id}-modal .emojiserver-entry .modal-emojiservername-label {
						width: 300px;
					}
					
					.${this.id}-modal .titles-entry .modal-titlestotal-label,
					.${this.id}-modal .titles-entry .modal-titlesglobal-label,
					.${this.id}-modal .titles-entry .modal-titleslocal-label,
					.${this.id}-modal .titles-entry .modal-titlesreplicate-label {
						cursor: pointer;
					}
					...
					.${this.id}-modal .titles-entry .modal-titleslocal-label,
					.${this.id}-modal .titles-entry .modal-titlesreplicate-label,
					.${this.id}-modal .emojiserver-entry .modal-emojitotal-label,
					.${this.id}-modal .emojiserver-entry .modal-emojiglobal-label,
					.${this.id}-modal .emojiserver-entry .modal-emojilocal-label,
					.${this.id}-modal .emojiserver-entry .modal-emojireplicate-label {
						text-align: center;
						width: 82px;
					}
		...
		hoverEmoji (picker) {
			$(picker)
				...
				.on("mouseenter." + this.name, ".emoji-item, .emojiItem-3l4Fa9", (e) => {
