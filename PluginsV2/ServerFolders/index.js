	...
	return class extends Plugin {
		...
		initialize() {
			if (typeof BDfunctionsDevilBro === "object") {
				...
				this.css = `
					...
					.${this.id}-modal .ui-icon-picker-icon {
						width: 70px;
						height: 70px;
					...
					.${this.id}-modal .ui-icon-picker-icon .ui-picker-inner {
						margin: 5px 5px;
						width: 60px;
						height: 60px;
					...
					.${this.id}-modal .ui-icon-picker-icon.selected .button-1qrA-N {
						display: none !important;
					}
					
					.${this.id}-modal .ui-icon-picker-icon .button-1qrA-N {
						position: absolute;
						top: -10px;
						right: -10px;
					}
					
					.${this.id}-modal .ui-icon-picker-icon.preview.nopic .ui-picker-inner {
		...
		addDragListener () {
			$(".guilds.scroller")
				...
				.on("mousedown." + this.name, "div.guild:not(.folder):not(.copy)", (e) => {
