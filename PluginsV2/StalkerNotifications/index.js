module.exports = (Plugin, Api, Vendor) => {
	...
	return class extends Plugin {
		...
		initialize() {
			if (typeof BDfunctionsDevilBro === "object") {
				...
				this.timeLogModalMarkup =
					`<span class="${this.id}timelog-modal DevilBro-modal">
				...
				this.css = `
					...
					.${this.id}timelog-modal .log-time {
						width: 100px;
					}
					.${this.id}timelog-modal .log-avatar {
						width: 35px;
						height: 35px;
					...
					.${this.id}-settingsmodal .avatar-list {
						display: flex;
						align-items: center;
						flex-wrap: wrap;
					}
					.${this.id}-settingsmodal .type-toast, .${this.id}-settingsmodal .type-desktop {
						border-radius: 3px;
						padding: 0 3px;
					}
					.${this.id}-settingsmodal .type-toast {
						background-color: #7289DA;
					}
					.${this.id}-settingsmodal .type-desktop {
						background-color: #43B581;
					}
					.${this.id}-settingsmodal .settings-avatar.desktop {
						border-color: #43B581;
					}
					.${this.id}-settingsmodal .settings-avatar {
						margin: 5px;
						width: 50px;
						height: 50px;
						background-size: cover;
					...
					.${this.id}-settingsmodal .settings-avatar.disabled {
						border-color: #36393F;
						filter: grayscale(100%) brightness(50%);
					}
					.${this.id}-settingsmodal .settings-avatar .button-1qrA-N {
						position: relative;
						top: -10px;
						right: -25px;
					}
					.${this.id}-settingsmodal .settings-avatar:not(:hover) .button-1qrA-N {
						opacity: 1;
					}
					.${this.id}-settingsmodal .disable-all {
						color: white;
		...
		getSettingsPanel () {
			...
			var settingshtml = `<div class="DevilBro-settings-inner">`;
