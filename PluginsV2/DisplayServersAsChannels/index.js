			...
			if (avatar) {
				...
				$(serverObj.div)
					...
					.on("mouseenter." + this.name, () => {
						BDfunctionsDevilBro.appendLocalStyle("HideAllToolTips" + this.name, `.tooltip {display: none !important;}`);
					})
					.on("mouseleave." + this.name, () => {
						BDfunctionsDevilBro.removeLocalStyle("HideAllToolTips" + this.name);
					});
			}
		}
