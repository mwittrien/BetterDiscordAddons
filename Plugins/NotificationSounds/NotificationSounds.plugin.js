    ...
    return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
        ...
        downloadLibrary() {
            BdApi.Net.fetch("https://mwittrien.github.io/BetterDiscordAddons/Library/0BDFDB.plugin.js").then(r => {
                ...
                if (!b)
                    ...
                    return require("fs").writeFile(require("path").join(BdApi.Plugins.folder, "0BDFDB.plugin.js"), b, _ => BdApi.UI.showToast("Finished downloading BDFDB Library", {
                            type: "success"
                        }));
            }).catch(error => {
     ...
     : (([Plugin, BDFDB]) => {
        ...
        const message1Types = {
            ...
            reply: {
                src: "./message2.mp3",
                name: "Message Mentioned (reply)"
            },
            role: {
                src: "./mention1.mp3",
                name: "Message Mentioned (role)"
            },
            everyone: {
                src: "./mention2.mp3",
                name: "Message Mentioned (@everyone)"
            },
            here: {
                src: "./mention3.mp3",
                name: "Message Mentioned (@here)"
            }
        };
                ...
                return new Promise((callback, errorCallback) => {
                    this._ensureAudio().then(audio => {
                        if (!duration && duration !== 0)
                            errorCallback(new Error("sound has no duration"));
                        audio.loop = false;
                        audio.play();
        ...
        return class NotificationSounds extends Plugin {
            onLoad() {
                ...
                this.defaults = {
                    volumes: {
                        globalVolume: {
                            value: 100,
                            description: "Global Notification Sounds Volume"
                        }
                    },
                    toggles: {
                        playIfMuted: {
                            value: false,
                            description: "Play Sounds If Muted"
                        }
                    }
                };
            ...
            onStart() {
                ...
                for (let key of soundKeys) {
                    ...
                    if (soundPackName != id && soundKeys.filter(n => n.indexOf(`./${soundPackName}`) > -1).length > 8) {
                        ...
                        if (!defaultAudios[soundPackName])
                            defaultAudios[soundPackName] = {};
                        ...
                        if (this.isSoundUsedAnywhere(id))
                            types[id] = {
                                name: name,
                                src: src,
                        ...
                        if (id == "message1" || id == "message3") {
                            ...
                            if (id == "message1")
                                for (let subType in message1Types)
                                    types[subType] = {
                ...
                for (let pack in defaultAudios)
                    defaultAudios[pack] = BDFDB.ObjectUtils.sort(defaultAudios[pack]);
                    ...
                    let change = _ => {
                        if (window.navigator.mediaDevices && window.navigator.mediaDevices.enumerateDevices) {
                            window.navigator.mediaDevices.enumerateDevices().then(enumeratedDevices => {
                                ...
                                if (deviceViaId && deviceViaIndex && deviceViaIndex.label != deviceViaId.name)
                                    deviceViaIndex = filteredDevices.find(d => d.label == deviceViaId.name);
                        ...
                        if (BDFDB.ObjectUtils.is(e.methodArguments[0]) && e.methodArguments[0].type == "MESSAGE_CREATE" && e.methodArguments[0].message) {
                            ...
                            if (message.author.id != BDFDB.UserUtils.me.id && !BDFDB.LibraryStores.RelationshipStore.isBlocked(message.author.id)) {
                                ...
                                if (!toggles.playIfMuted) {
                                    if ((isThread && BDFDB.LibraryStores.JoinedThreadsStore.isMuted(channel.id)) || (!isThread && BDFDB.LibraryStores.UserGuildSettingsStore.isGuildOrCategoryOrChannelMuted(guildId, channel.id)))
                                        return;
                                }
                                ...
                                if (isCurrent && BDFDB.LibraryStores.NotificationSettingsStore.getNotifyMessagesInSelectedChannel()) {
                                    ...
                                    if (BDFDB.LibraryModules.MentionUtils.isRawMessageMentioned({
                                            rawMessage: message,
                                            userId: BDFDB.UserUtils.me.id
                                        })) {
                                        if (message.mentions.length && !this.isSuppressMentionsEnabled(guildId, channel.id))
                                            for (const mention of message.mentions)
                                                if (mention.id == BDFDB.UserUtils.me.id) {
                ...
                BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.DesktopNotificationUtils, "showNotification", {
                    before: e => {
                        let soundObjIndex = Array.from(e.methodArguments).findIndex(n => n && n.sound);
                        if (soundObjIndex && e.methodArguments[soundObjIndex].sound.includes("message"))
                            e.methodArguments[soundObjIndex].sound = null;
                    }
                });
                    ...
                    let cancel = BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.SoundUtils, "createSound", {
                        after: e => {
                            if (e.returnValue && e.returnValue.constructor && e.returnValue.constructor.prototype && typeof e.returnValue.constructor.prototype.play == "function") {
                                cancel();
                                BDFDB.PatchUtils.patch(this, e.returnValue.constructor.prototype, ["play", "loop", "playWithListener"], {
                                    instead: e2 => {
                                        let type = e2.instance && e2.instance.name;
                                        if (type && choices[type]) {
                                            e2.stopOriginalMethodCall();
                                            if (type == "message1" || type == "message3")
                                                BDFDB.TimeUtils.timeout(_ => {
                                                    let called = false;
                                                    for (let subType of[type].concat(Object.keys(message1Types)))
                                                        if (firedEvents[subType]) {
                                                            delete firedEvents[subType];
                                                            called = true;
                                                            break;
                                                        }
                                                    if (!called)
                                                        return this.playAudio(type, e2.originalMethodName, e2.instance.duration);
                                                });
                                            else
                                                return this.playAudio(type, e2.originalMethodName, e2.instance.duration);
                                        } else
                                ...
                                BDFDB.PatchUtils.patch(this, e.returnValue.constructor.prototype, "stop", {
                                    after: e2 => {
                                        let type = e2.instance && e2.instance.name;
                                        if (type && createdAudios[type])
                                            createdAudios[type].stop();
                                    }
                                });
                            }
                        }
            ...
            onStop() {
                for (let type in createdAudios)
                    if (createdAudios[type])
                        createdAudios[type].stop();
            }

            getSettingsPanel(collapseStates = {}) {
                let successSavedAudio = data => {
                    BDFDB.NotificationUtils.toast(`Sound ${data.sound} was added to category ${data.category}.`, {
                        type: "success"
                    });
                    if (!audios[data.category])
                        audios[data.category] = {};
                ...
                return settingsPanel = BDFDB.PluginUtils.createSettingsPanel(this, {
                    ...
                    children: _ => {
                                    ...
                                    BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
                                        ...
                                        children: [
                                            ...
                                            BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Button, {
                                                ...
                                                onClick: _ => {
                                                    for (let input of settingsPanel.props._node.querySelectorAll(".input-newsound " + BDFDB.dotCN.input))
                                                        if (!input.value || input.value.length == 0 || input.value.trim().length == 0)
                                                            return BDFDB.NotificationUtils.toast("Fill out all Fields to add a new Sound", {
                                                                type: "danger"
                                                            });
                                                    ...
                                                    if (value.indexOf("http") == 0)
                                                        BDFDB.LibraryRequires.request(value, (error, response, result) => {
                                                            if (response) {
                                                                let type = response.headers["content-type"];
                                                                if (type && (type.indexOf("octet-stream") > -1 || type.indexOf("audio") > -1 || type.indexOf("video") > -1))
                                                                    return successSavedAudio({
                                                                        category,
                                                                        sound,
                                                                        source: value
                                                                    });
                                                            }
                                                            BDFDB.NotificationUtils.toast("Use a valid direct link to a video or audio source, they usually end on something like .mp3, .mp4 or .wav", {
                                                                type: "danger"
                                                            });
                                                        });
                                            ...
                                            children: [
                                                ...
                                                ["mute", "streamMute", "invisibleMute"].some(n => types[type][n] !== null) && BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Clickable, {
                                                    onClick: event => BDFDB.ContextMenuUtils.open(this, event, BDFDB.ContextMenuUtils.createItem(BDFDB.LibraryComponents.MenuItems.MenuGroup, {
                                                            children: [{
                                                                    key: "mute",
                                                                    label: ["Mute in", BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.StatusComponents.Status, {
                                                                            style: {
                                                                                marginLeft: 6
                                                                            },
                                                                            size: 12,
                                                                    ...
                                                                    label: ["Mute in", BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.StatusComponents.Status, {
                                                                            ...
                                                                            size: 12,
                                                                            status: BDFDB.LibraryComponents.StatusComponents.Types.INVISIBLE
                                                                        })]
                                                                }, {
                                                                    key: "streamMute",
                                                                    label: ["Mute while", BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.StatusComponents.Status, {
                                                                            style: {
                                                                                marginLeft: 6
                                                                            },
                                                                            size: 12,
                                                ...
                                                BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex.Child, {
                                                    ...
                                                    children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.FormItem, {
                                                        ...
                                                        children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Select, {
                                                            ...
                                                            options: Object.keys(audios).map(name => ({
                                                                    value: name,
                                                                    label: name
                                                                })),
                                                            searchable: true,
                                                ...
                                                BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex.Child, {
                                                    ...
                                                    children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.FormItem, {
                                                        ...
                                                        children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Select, {
                                                            ...
                                                            options: Object.keys(audios[choices[type].category] || {}).map(name => ({
                                                                    value: name,
                                                                    label: name
                                                                })),
                                                            searchable: true,
                        ...
                        let removeableCategories = [{
                                value: removeAllKey,
                                label: BDFDB.LanguageUtils.LanguageStrings.ALL
                            }
                        ].concat(Object.keys(audios).filter(category => !(defaultAudios[category] && !Object.keys(audios[category] || {}).filter(sound => defaultAudios[category][sound] === undefined).length)).map(name => ({
                                    value: name,
                                    label: name
                                })));
                        let removeableSounds = {};
                        for (let category of removeableCategories)
                            removeableSounds[category.value] = [{
                                    value: removeAllKey,
                                    label: BDFDB.LanguageUtils.LanguageStrings.ALL
                                }
                            ].concat(Object.keys(audios[category.value] || {}).filter(sound => !(defaultAudios[category.value] && defaultAudios[category.value][sound] !== undefined)).map(name => ({
                                        value: name,
                                        label: name
                                    })));
                        settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
                                            ...
                                            children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.FormItem, {
                                                ...
                                                children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Select, {
                                                    ...
                                                    onChange: (category, instance) => {
                                                        let soundSelectIns = BDFDB.ReactUtils.findOwner(BDFDB.ReactUtils.findOwner(instance, {
                                                                    name: ["BDFDB_Modal", "BDFDB_SettingsPanel"],
                                                                    up: true
                                                                }), {
                                                                key: "REMOVE_SOUND"
                                                            });
                                        ...
                                        BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex.Child, {
                                            ...
                                            children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Button, {
                                                ...
                                                onClick: (event, instance) => {
                                                    let wrapperIns = BDFDB.ReactUtils.findOwner(instance, {
                                                        name: ["BDFDB_Modal", "BDFDB_SettingsPanel"],
                                                        up: true
                                                    });
                                                    let categorySelectIns = BDFDB.ReactUtils.findOwner(wrapperIns, {
                                                        key: "REMOVE_CATEGORY"
                                                    });
                                                    let soundSelectIns = BDFDB.ReactUtils.findOwner(wrapperIns, {
                                                        key: "REMOVE_SOUND"
                                                    });
                                                    if (categorySelectIns && soundSelectIns) {
                                                        ...
                                                        if (catAll)
                                                            soundAmount = BDFDB.ArrayUtils.sum(Object.keys(audios).map(category => Object.keys(audios[category] || {}).filter(sound => !(defaultAudios[category] && defaultAudios[category][sound] !== undefined)).length));
                                                        else if (soundAll)
                                                            soundAmount = Object.keys(audios[categorySelectIns.props.value] || {}).filter(sound => !(defaultAudios[categorySelectIns.props.value] && defaultAudios[categorySelectIns.props.value][sound] !== undefined)).length;
                                                        else if (audios[categorySelectIns.props.value][soundSelectIns.props.value])
                                                            soundAmount = 1;

                                                        if (soundAmount)
                                                            BDFDB.ModalUtils.confirm(this, `Are you sure you want to delete ${soundAmount} added Sound${soundAmount == 1 ? "" : "s"}?`, _ => {
                                                                if (catAll)
                                                                    BDFDB.DataUtils.remove(this, "audios");
                                                                else if (soundAll)
                                                                    BDFDB.DataUtils.remove(this, "audios", categorySelectIns.props.value);
                                                                else {
                                                                    delete audios[categorySelectIns.props.value][soundSelectIns.props.value];
                                                                    if (BDFDB.ObjectUtils.isEmpty(audios[categorySelectIns.props.value]))
                                                                        delete audios[categorySelectIns.props.value];
                                                                    BDFDB.DataUtils.save(audios, this, "audios");
                                                                }
                                                                this.loadAudios();
                                                                this.loadChoices();
                                                            ...
                                                            BDFDB.NotificationUtils.toast("No Sounds to delete", {
                                                                type: "danger"
                                                            });
                                                    }
                                                },
            ...
            loadChoices() {
                ...
                for (let type in types) {
                    let choice = loadedChoices[type] || {},
                    soundFound = false;
                    for (let category in audios)
                        if (choice.category == category)
                            for (let sound in audios[category])
                                if (choice.sound == sound) {
                                    soundFound = true;
                                    break;
                                }
                    if (!soundFound)
            ...
            playAudio(type, functionCall = "play", duration = 0) {
                ...
                if (this.dontPlayAudio(type) || BDFDB.LibraryStores.StreamerModeStore.disableSounds)
                    return;
                if (createdAudios[type])
                    createdAudios[type].stop();
