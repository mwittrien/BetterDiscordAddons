...
(() => {
	...
	BDFDB.readServerList = function () {
		var found = [], ins = BDFDB.getOwnerInstance({node:document.querySelector(BDFDB.dotCN.guilds), name:'Guild', all:true, noCopies:true, depth:99999999, time:99999999});
		for (let info in ins) if (ins[info].props && ins[info].props.guild && ins[info]._reactInternalFiber.child) found.push(Object.assign(new ins[info].props.guild.constructor(ins[info].props.guild), {div:BDFDB.React.findDOMNodeSafe(ins[info]), instance:ins[info]}));
		return found;
	};
