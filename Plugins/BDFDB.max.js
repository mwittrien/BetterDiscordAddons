...
(() => {
	...
	BDFDB.createNotificationsBar = function (text, options = {}) {
		...
		if (!layers) return;
		var id = Math.round(Math.random() * 10000000000000000);
		var notice = BDFDB.htmlToElement(`<div class="${BDFDB.disCNS.notice + BDFDB.disCNS.size14 + BDFDB.disCNS.weightmedium + BDFDB.disCN.height36} BDFDB-notice notice-${id}"><div class="${BDFDB.disCN.noticedismiss}" style="height:36px !important; top: 0 !important; right 0 !important;"></div><span class="notice-message"></span></div>`);
