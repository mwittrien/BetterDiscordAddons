...
if (window.BDFDB && BDFDB.ObserverUtils && typeof BDFDB.ObserverUtils.killAll == "function") BDFDB.ObserverUtils.killAll(BDFDB);
...
(() => {
	...
	BDFDB.PluginUtils.init = function (plugin) {
		BDFDB.PluginUtils.clearStartTimeout(plugin);
	...
	BDFDB.PluginUtils.clear = function (plugin) {
		...
		BDFDB.ObserverUtils.killAll(plugin);
	...
	BDFDB.PluginUtils.clearStartTimeout = function (plugin) {
	...
	BDFDB.ObserverUtils = {};
	BDFDB.ObserverUtils.add = function (plugin, eleOrSelec, observer, config = {childList: true}) {
	...
	BDFDB.ObserverUtils.kill = function (plugin, observer) {
		if (BDFDB.ObjectUtils.is(plugin) && !BDFDB.ObjectUtils.isEmpty(plugin.observers)) {
			...
			if (!observername || !Array.isArray(plugin.observers[observername])) return;
			for (let instance of plugin.observers[observername]) instance.disconnect();
			delete plugin.observers[observername];
		}
	};
	BDFDB.ObserverUtils.killAll = function (plugin) {
		if (BDFDB.ObjectUtils.is(plugin) && !BDFDB.ObjectUtils.isEmpty(plugin.observers)) {
		...
		if (BDFDB.ObjectUtils.is(plugin) && BDFDB.ObjectUtils.is(plugin.patchModules)) {
			...
			function checkForInstance(classname, type) {
				...
				if (!instancefound) {
					...
					BDFDB.ObserverUtils.add(plugin, BDFDB.dotCN.appmount, {name:"checkForInstanceObserver", instance:instanceobserver, multi:true
	...
	var BDFDBpatchContextMenuModulePlugin = (plugin, type, module) => {
	...
	var BDFDBpatchPopoutModulePlugin = (plugin, type, module) => {
		if (module && module.prototype) BDFDB.WebModules.patch(module.prototype, "render", plugin, {after: e => {
	...
	var BDFDBpatchContextMenuModuleLib = (module, repatch) => {
	...
	var BDFDBpatchPopoutModuleLib = (module, repatch) => {
		if (module && module.prototype) {
			BDFDB.WebModules.patch(module.prototype, "componentDidMount", BDFDB, {after: e => {
	...
	var getDiscordClass = (item, selector) => {
	...
	addContextListener(BDFDB);
	
	BDFDB.ObserverUtils.add(BDFDB, document.querySelector(BDFDB.dotCN.itemlayercontainer), {name:"layerObserverBDFDB", instance:
		new MutationObserver(changes => {changes.forEach(change => {change.addedNodes.forEach(node => {
	...
	BDFDB.addObserver = BDFDB.ObserverUtils.add;
	BDFDB.killObservers = BDFDB.ObserverUtils.killAll;
