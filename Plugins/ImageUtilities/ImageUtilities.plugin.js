							...
							e.node.addEventListener("mousedown", event => {
								...
								this.addListener("keydown", "Zoom", event2 => {
									if (!document.contains(e.node)) this.cleanupListeners("Zoom");
										...
										if (event2.keyCode == 187 && (zoomLevel + 0.5*zoomLevel) <= this.defaults.zoomSettings.zoomLevel.maxValue) {
											zoomLevel += 0.5*zoomLevel;
											lens.update();
										}
										else if (event2.keyCode == 189 && (zoomLevel - 0.5*zoomLevel) >= this.defaults.zoomSettings.zoomLevel.minValue) {
											zoomLevel -= 0.5*zoomLevel;
											lens.update();
										}
									}
