 ...
 * @updateUrl https://raw.githubusercontent.com/whuihuan/BetterDiscordAddons/master/Plugins/GoogleTranslateOption/GoogleTranslateOption.plugin.js
 */
...
module.exports = (_ => {
	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		const translationEngines = {
			...
			baidu: 						{name: "Baidu", 			auto: true, 	funcName: "baiduTranslate", 			languages: ["en","es","fr","id","ja","ko","th","vi","zh-CN","zh-TW"]}
		};
						...
						try {
							...
							if (!res.error_code) {
								...
								if (message && message.length > 0 && res.from != res.to) {
									...
									BDFDB.NotificationUtils.toast(`The current language is the same as the target language, no translation required `, {
										position: "center"
									});
									callback("");
								}
								...
								BDFDB.NotificationUtils.toast(`${this.labels.toast_translating_failed}. ${this.labels.toast_translating_tryanother}. ${res.error_code} : ${res.error_msg}.`, {
									type: "danger",
									position: "center"
								});
								callback("");
