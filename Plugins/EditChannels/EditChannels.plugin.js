...
class EditChannels {
	...
	processChannelAutoComplete (e) {
		if (e.instance.state.autocompleteType == "CHANNELS" && BDFDB.ArrayUtils.is(e.instance.state.autocompletes.channels) && e.instance.props.channel && e.instance.props.channel.guild_id) {
			...
			for (let id in channels) if (channels[id] && channels[id].name) {
				...
				if (BDFDB.ChannelUtils.isTextChannel(channel) && channel.guild_id == e.instance.props.channel.guild_id) channelarray.push(Object.assign({lowercasename:channels[id].name.toLowerCase(),lowercasecatname:catdata && catdata.name && catdata.name.toLowerCase(),channel,category,catdata},channels[id]));
			}
	...
	processInviteCard (e) {
		let channel = BDFDB.ReactUtils.getValue(e.instance, "props.invite.channel");
		if (channel && BDFDB.DataUtils.get(this, "settings", "changeInInviteLog")) {
			if (!e.returnvalue) e.instance.props.invite.channel = this.getChannelData(channel.id);
			else {
				let [children, index] = BDFDB.ReactUtils.findChildren(e.returnvalue, {props:[["className", BDFDB.disCN.guildsettingsinvitechannelname]]});
				if (index > -1) this.changeChannelColor(children[index], channel.id);
			}
		}
	}
		...
		if (BDFDB.DataUtils.get(this, "settings", "changeInRecentMentions")) {
			...
			if (index > -1 && children[index].props.children && BDFDB.ArrayUtils.is(children[index].props.children[0])) for (let dividerAndMessage of children[index].props.children[0]) if (dividerAndMessage) {
				...
				if (channel && BDFDB.ChannelUtils.isTextChannel(channel)) {
					...
					this.changeChannelColor(dividerAndMessage[0].props.children[0], channel.id);
				}
			}
		}
	}
	...
	processMessageContent (e) {
		if (BDFDB.ArrayUtils.is(e.instance.props.content) && BDFDB.DataUtils.get(this, "changeInMentions")) for (let ele of e.instance.props.content) {
								...
								if (name || color) {
									...
									ele.props.children = (...args) => {
										...
										if (color) {
											...
											let onMouseEnter = renderedChildren.props.onMouseEnter;
											...
											let onMouseLeave = renderedChildren.props.onMouseLeave;
