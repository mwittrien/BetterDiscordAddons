...
module.exports = ((_) => {
  ...
  return (window.Lightcord && !Node.prototype.isPrototypeOf(window.Lightcord)) ||
    (window.LightCord && !Node.prototype.isPrototypeOf(window.LightCord)) ||
    (window.Astra && !Node.prototype.isPrototypeOf(window.Astra))
    ? class {
        getName() {
        ...
        getDescription() {
          return 'Do not use LightCord!';
        }
        load() {
          BdApi.alert(
            'Attention!',
            'By using LightCord you are risking your Discord Account, due to using a 3rd Party Client. Switch to an official Discord Client (https://discord.com/) with the proper BD Injection (https://betterdiscord.app/)'
          );
        }
        start() {}
        stop() {}
      }
    : !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started)
    ? class {
        getName() {
          return config.info.name;
        }
    ...
    : (([Plugin, BDFDB]) => {
        ...
        const ReadAllButtonComponent = class ReadAllButton extends BdApi.React.Component {
          ...
          getGuilds() {
            return BDFDB.LibraryModules.FolderStore.getFlattenedGuilds()
              .map((g) => g.id)
              .filter((n) => n);
          }
          getUnread() {
            return this.getGuilds().filter(
              (id) => BDFDB.LibraryModules.UnreadGuildUtils.hasUnread(id) || BDFDB.LibraryModules.UnreadGuildUtils.getMentionCount(id) > 0
            );
          }
          getPinged() {
          ...
          getPingedDMs() {
            return BDFDB.LibraryModules.ChannelStore.getSortedPrivateChannels()
              .map((c) => c.id)
              .filter((id) => id && BDFDB.LibraryModules.UnreadChannelUtils.getMentionCount(id) > 0);
          }
          render() {
        ...
        return class ReadAllNotificationsButton extends Plugin {
          ...
          getSettingsPanel(collapseStates = {}) {
            ...
            settingsItems.push(
              BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
                ...
                children: Object.keys(this.defaults.general)
                  .map((key) =>
                  ...
                  .concat(
                    BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsPanelList, {
                      ...
                      first: false,
                      last: true,
                      children: Object.keys(this.defaults.batch).map((key) =>
                        BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsSaveItem, {
                          type: 'Switch',
                          plugin: this,
                          keys: ['batch', key],
          ...
          processRecentMentions(e) {
            if (e.instance.props.header && e.instance.props.header.props)
              e.instance.props.header.props.messages = e.returnvalue.props.messages;
          }

          processRecentsHeader(e) {
            if (this.settings.general.addClearButton && e.instance.props.tab == BDFDB.LibraryModules.InboxUtils.InboxTab.MENTIONS)
                ...
                BDFDB.ReactUtils.createElement('div', {
                  children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.TooltipContainer, {
                    ...
                    children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Clickable, {
                      className:
                        BDFDB.disCNS.messagespopoutbutton +
                        BDFDB.disCNS.messagespopoutbuttonsecondary +
                        BDFDB.disCN.messagespopoutbuttonsize32,
                        ...
                        let clear = (_) => {
                          if (clearing)
                            return BDFDB.NotificationUtils.toast(
                              `${this.labels.toast_alreadyclearing} - ${BDFDB.LanguageUtils.LibraryStrings.please_wait}`,
                              { type: 'danger' }
                            );
